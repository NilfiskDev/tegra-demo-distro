...
DATAPART_FILE_IMAGE ?= "data-partition-image"
DATAPART_FILE_IMAGE_FULL_NAME ?= "${DEPLOY_DIR_IMAGE}/${DATAPART_FILE_IMAGE}-${MACHINE}.ext4"
DATAPART_FILE ?= "${@'datapart.ext4' if d.getVar('DATAPART_FILE_IMAGE') else ''}"
DATAPART_SIZE ?= "1106127360"
...
tegraflash_create_flash_config() {
    ...
    sed \
        ...
        -e"s,APPSIZE,${IMAGE_APP_PART_DEFAULT_SIZE}," \
        -e"s,RECROOTFSSIZE,${RECROOTFSSIZE}," \
        -e"s,APPUUID_b,," -e"s,APPUUID,," \
        -e"s,DATAPART_SIZE,${DATAPART_SIZE}," \
        -e"s,DATAPART_FILE,${DATAPART_FILE}," \
...
create_tegraflash_pkg() {
    ...
    if [ -n "${DATAPART_FILE}" ]; then
        cp "${DATAPART_FILE_IMAGE_FULL_NAME}" ./${DATAPART_FILE}
    fi
    if [ -n "${IMAGE_TEGRAFLASH_INITRD_FLASHER}" ]; then
...
do_image_tegraflash_tar[depends] += "dtc-native:do_populate_sysroot coreutils-native:do_populate_sysroot \
                                 ...
                                 data-partition-image:do_image_complete \
